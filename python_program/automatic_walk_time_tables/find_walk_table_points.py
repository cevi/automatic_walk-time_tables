...
class WalkTablePointFinder:
    ...
    def select_waypoints(self, walk_point_limit=21):
        ...
        total_distance, pts_step_1 = self.preselection_step()

        self.logger.debug("Preselection returned %d points", len(pts_step_1))

        pts_step_2 = self.remove_unnecessary_points(pts_step_1)

        self.logger.debug("Remove unnecessary points returned %d points", len(pts_step_2))

        pts_step_3 = self.reduce_number_of_points(pts_step_2, walk_point_limit)

        self.logger.debug("Reduce number of points returned %d points", len(pts_step_3))
        self.logger.debug("Total distance: %f km", total_distance)

        return total_distance, pts_step_2, pts_step_3


    def reduce_number_of_points(self, pts_step_2: List[Tuple[int, GPXTrackPoint]], walk_point_limit: int):
        """
        ...
        while len(pts_step_3) > walk_point_limit:
            ...
            for pt_C in pts_step_3:
                ...
                if pt_A is not None and pt_B is not None:
                    ...
                    m, b = self.calc_secant_line(pt_A, pt_C)
                    secant_elev = self.calc_secant_elevation(m, b, pt_B)
                    ...
                    if abs(secant_elev - pt_B[1].elevation) < drv_limit:
                        ...
                        for pt in list(filter(lambda p: pt_A[0] < p[0] < pt_C[0], pts_dropped)):
                            secant_elev = self.calc_secant_elevation(m, b, pt)
    ...
    def remove_unnecessary_points(self, pts_step_1: List[Tuple[int, GPXTrackPoint]]):
        """
    ...
    def preselection_step(self):
        ...
        # (1) Preselection
        for track in self.raw_gpx_data.tracks:
            for segment in track.segments:
    ...
    def calc_secant_elevation(self, m: float, b: float, pt_B: Tuple[int, GPXTrackPoint]):
        """
    ...
    def calc_secant_line(self, pt_A: Tuple[int, GPXTrackPoint], pt_C: Tuple[int, GPXTrackPoint]):
        """
