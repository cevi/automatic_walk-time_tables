...
export class ExportSettingsComponent {
  ...
  constructor(
    ...
    this.options = fb.group({
      ...
      departure_time: new UntypedFormControl(
      ...
      automatic_waypoint_selection: new FormControl<boolean>(
        this.mapAnimator.auto_waypoints,
      ),
    });
    ...
    try {
      if (
        localStorage['form_values'] &&
        this.isJsonString(localStorage['form_values'])
      ) {
  ...
  public download_map() {
    ...
    this.mapAnimator
      .download_map(settings)
      .then((uuid) => this.router.navigate(['pending', uuid]))
