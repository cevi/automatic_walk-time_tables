    ...
    try:
        ...
        for lat, lon in lv95_coords:
            ...
            for n in swiss_names:
                ...
                if n.object_type in ("Trockenrinne", "Druckleitung einfach", "Druckleitung mehrfach", "Druckstollen"):
                    continue
                ...
                if "fliessgewaesser (keine brücke)" in name_lower:
                    n.name = n.name.replace("Fliessgewaesser (Keine Brücke)", "Gewässerquerung")
                    name_lower = n.name.lower()
                
                prio = 0
                max_d = 50
                
                if "sac" in name_lower or "hütte" in name_lower or "abgelegener gasthof" in name_lower or "abgelegener gasthof" in obj_lower:
                    prio, max_d = 100, 200
                elif "pass" in name_lower or "pass" in obj_lower:
                    prio, max_d = 90, 200
                elif "see" in obj_lower or "lac " in name_lower or "lai " in name_lower or "see " in name_lower or name_lower.endswith("see") or name_lower.endswith("lac") or name_lower.endswith("lai"):
                    prio, max_d = 85, 250
                elif "hauptgipfel" in obj_lower or "gipfel" in obj_lower or "gipfelkreuz" in obj_lower:
                    prio, max_d = 80, 150
                elif "staudamm" in obj_lower or "wehr" in obj_lower or "staumauer" in obj_lower:
                    prio, max_d = 70, 100
                    ...
                    prio, max_d = 50, 120
                elif "gebäude" in obj_lower or "turm" in obj_lower or "kapelle" in obj_lower or "ruine" in obj_lower or "historische baute" in obj_lower:
                    prio, max_d = 45, 10
                elif "nationalpark" in obj_lower:
                    prio, max_d = 30, 50
                elif "kraftwerkareal" in obj_lower or "abwasserreinigungsareal" in obj_lower or "areal" in obj_lower:
                    prio, max_d = 30, 30
                elif "kreuzung hochspannungsleitung" in obj_lower or "fliessgewaesser" in obj_lower or "seilbahn" in obj_lower:
                    prio, max_d = 20, 20
                elif "sportplatz" in obj_lower or "rodelbahn" in obj_lower or "skisprungschanze" in obj_lower:
                    prio, max_d = 15, 20
                elif "kreuzung" in obj_lower or "weggabelung" in obj_lower or "kreisel" in obj_lower or "wegende" in obj_lower:
                    prio, max_d = 10, 25
            ...
            for c in candidates_log[:7]:
                logger.info("  [%s] %s (%s) - dist: %sm, prio: %s, max_d: %s", "VALID" if c["valid"] else " EXCL", c["name"], c["type"], c["dist"], c["prio"], c["max_d"])

            # Fallback if no object was within its max_d radius
            if not best_n:
                valid_fallbacks = [n for n in swiss_names if n.name and str(n.name).strip() != ""]
                if valid_fallbacks:
            ...
            is_lake = "see" in obj_lower or "lac " in name_lower or "lai " in name_lower or "see " in name_lower or name_lower.endswith("see") or name_lower.endswith("lac") or name_lower.endswith("lai")
            if is_lake and f"({int(swiss_name.h)})" not in swiss_name.name and swiss_name.h > 0:
            ...
            if swiss_name.object_type == "Kotierter Punkt" or swiss_name.name.startswith("Punkt "):
                best_flurname = None
