        ...
        with fiona.open(shp_file) as src:
            ...
            for obj in src:
                ...
                if obj_type == 'Standardknoten':
                    ...
                    adjoined_streets = list(path_index.intersection(coordinates=search_area, objects='raw'))
                    ...
                    is_roundabout = any([obj['properties']['KREISEL'] == 'Wahr' for obj in adjoined_streets])

                    # TODO: prefer nearest Siedlungsnamen over Flurnamen if no street name is available
                    if not len(street_names):
                        flurnamen: SwissName = \
                            next(flurname_index.nearest(coordinates=geo[0:2], num_results=1, objects='raw'))
                    ...
                    if street_count >= 3 and len(street_names) >= 2:
                        ...
                        swiss_name = SwissName(name=name, object_type='Kreisel', x=int(geo[0]), y=int(geo[1]),
                                               h=geo[2])
                        self.index.insert(id=0, coordinates=geo[0:2], obj=swiss_name)

                    elif street_count >= 3:
                        name = "Weggabelung{}".format(list_of_street_names)
                        swiss_name = SwissName(name=name, object_type='Weggabelung', x=int(geo[0]), y=int(geo[1]),
                                               h=geo[2])
                        self.index.insert(id=0, coordinates=geo[0:2], obj=swiss_name)
                    ...
                    # Can be done by adding a minimal street length
                        ...
                        swiss_name = SwissName(name=name, object_type='Wegende', x=int(geo[0]), y=int(geo[1]),
                                               h=geo[2])
                        self.index.insert(id=0, coordinates=geo[0:2], obj=swiss_name)
        ...
        with fiona.open(shp_file) as src:
            ...
            for obj in src:
                ...
                if obj['properties']['KUNSTBAUTE'] not in ('Keine', 'k_W'):
                    ...
                    for pkt in geo:
                        swiss_name = SwissName(name=name, object_type=obj_type, x=int(pkt[0]), y=int(pkt[1]),
                                               h=int(pkt[2]))
                        self.index.insert(id=0, coordinates=(pkt[0], pkt[1]), obj=swiss_name)
