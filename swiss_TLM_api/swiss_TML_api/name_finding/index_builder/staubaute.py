...
class Staubaute(IndexBuilder):
    """
    Inserts objects like Wehr, Staudamm from TLM_STAUBAUTE.
    """

    def load(self):
        shp_file = self.base_path + "swissTLM3D_TLM_STAUBAUTE.shp"
        try:
            with fiona.open(shp_file) as src:
                for obj in src:
                    obj_type = obj["properties"].get("OBJEKTART", "")
                    if obj_type not in ["Wehr", "Staudamm", "Staumauer"]:
                        continue
