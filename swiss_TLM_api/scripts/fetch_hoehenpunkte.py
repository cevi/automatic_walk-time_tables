...
def main():
    ...
    schema = {
        'geometry': 'Point',
        'properties': {'HOEHE': 'int'}
    }
    
    total_written = 0
    with fiona.open(OUTPUT_SHP, 'w', driver='ESRI Shapefile', crs="EPSG:2056", schema=schema) as output:
        for idx, download_url in enumerate(gdb_urls):
            print(f"[INFO] Processing tile {idx+1}/{len(gdb_urls)} | Points so far: {total_written}")
            zip_path = "/tmp/smv25_tile.zip"
            extract_dir = "/tmp/smv25_extracted"
            
            if os.path.exists(zip_path): os.remove(zip_path)
            if os.path.exists(extract_dir): shutil.rmtree(extract_dir)
            if os.path.exists("/tmp/extract_temp"): shutil.rmtree("/tmp/extract_temp")
            ...
            if gdb_dir:
                try:
                    ...
                    target_layer = next((l for l in layers if "HOEHENKOTE" in l.upper() and "ANNO" not in l.upper() and "MASK" not in l.upper()), None)
    ...
    # Final cleanup
    if os.path.exists("/tmp/smv25_tile.zip"): os.remove("/tmp/smv25_tile.zip")
    if os.path.exists("/tmp/smv25_extracted"): shutil.rmtree("/tmp/smv25_extracted")
    if os.path.exists("/tmp/extract_temp"): shutil.rmtree("/tmp/extract_temp")
