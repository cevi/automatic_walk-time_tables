...
class MapImage:
    ...
    def __init__(self, lv03_pkt, zoom_level):
        self.img = Image.new(
            "RGBA", (self.TILE_WIDTH * self.x_count, self.TILE_WIDTH * self.y_count)
        )

        # calc the number of the bottom left tile
        tile_base_row = (
            math.floor((lv03_pkt[0] - 2420_000) / self.TILE_SIZES[zoom_level]) - 1
        )
        tile_base_col = (
            math.floor((1350_000 - lv03_pkt[1]) / self.TILE_SIZES[zoom_level]) + 2
        )
        base_url = (
            ...
            + "/"
            + str(self.ZOOM_LEVELS[zoom_level])
            + "/"
        )
        ...
        self.lv03_min = (
            tile_base_row * self.TILE_SIZES[zoom_level] + 2420_000,
    ...
    def calc_img_coord(self, image_size, lv03_point):
        ...
        img_y = (
            image_size[1] - (lv03_point[1] - self.lv03_min[1]) * self.pixels_per_meter
        )
